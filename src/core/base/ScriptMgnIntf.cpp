...
// デバッガ実行されている時、Visual Studio で行ジャンプする時の指定をデバッグ出力に出して、break で停止する
				if( ::IsDebuggerPresent() ) {
					std::wstring debuglile( std::wstring(L"2>")+path.AsStdString()+L"("+std::to_wstring((__int64)lineno)+L"): error :" + errstr.AsStdString() );
				...
				if( TVPGetCommandLine(TJS_W("-exceptionexe"), &val) )
				{
					...
					if( TVPGetCommandLine(TJS_W("-exceptionarg"), &val) )
					{
						...
						if( !exepath.IsEmpty() && !arg.IsEmpty() ) {
							...
							str = ApplicationSpecialPath::ReplaceStringAll( str, std::wstring(L"%line%"), std::to_wstring((__int64)lineno) );
